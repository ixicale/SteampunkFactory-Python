...
REQUESTS = get_file_content(
    "data/_SELECT_sun_nombre_as_unidad_negocio_ss_idservice_servicio_as_id_202006041734.json"
)
SQL_LIST = json.loads(REQUESTS)["data"]
BASE = "https://s3.amazonaws.com/webfiles_palace/clever/products/assets"
TEMPLATE_INS = """-- idservice_servicio: {id_servicio}
...
def contar(path, **item):
    ...
    if response.status_code == 200:
        ...
        msg = """REASON=> {reason}\tCODE=> {status_code}\tURL=>{url}\tTEXT=>{text}""".format(
        ...
        for name in [_service, _codigo_produccion]:
            for exten in ["png", "PNG", "JPG", "jpg"]:
                path = "{base}/{name}.{exten}".format(
                    name=name, exten=exten, base=BASE
                )
